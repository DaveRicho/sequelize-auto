      ...
      _.each(fields, function(field, i){
        ...
        _.each(fieldAttr, function(attr, x){
         ...
         // conditionally add dateformatting for SMALLDATETIME strings
          // get: function(){ 
          // if (!Moment.utc(this.getDataValue('client_first_earliest_start_date')).isValid()) {
          //   return null;
          // }
          // // return Moment.utc(this.getDataValue('client_date_recent_full_soa')).format('YYYY-MM-DD HH:MM:SS');.
         //   }
          ...
          if (attr === "type") {
            ...
            if (field_type2.match(/^date/) || field_type2.match(/^smalldatetime/)) {
              ...
              text[table] += "\n";
              text[table] += spaces + spaces + spaces + "if (!Moment.utc(this.getDataValue('client_first_earliest_start_date')).isValid()) {";
              text[table] += "\n";
              text[table] += spaces + spaces + spaces + spaces + spaces + "return null;";
              text[table] += "\n";
              text[table] += spaces + spaces + spaces + "}";
              text[table] += spaces + spaces + spaces + spaces + spaces + `return Moment.utc(this.getDataValue('${fieldName}')).format('YYYY-MM-DD HH:MM:SS');`;
              text[table] += "\n";
              text[table] += spaces + spaces + spaces + "}";
              text[table] += ",\n";
            }
